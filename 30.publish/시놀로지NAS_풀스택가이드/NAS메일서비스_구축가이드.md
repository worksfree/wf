...
## 배경 — 자체 도메인 이메일이 왜 차단될까?

스타트업이 자체 도메인으로 직접 이메일 서버를 운영하면 대부분의 메일이 스팸함으로 들어가거나 차단되는 심각한 문제를 겪습니다. Google(Gmail), Yahoo, 네이버 등 대형 메일 서비스 제공사(ISP)들은 스팸을 차단하기 위해 발신인의 보안 인증과 도메인 평판을 매우 엄격하게 심사하기 때문입니다.

### 자체 도메인 이메일이 도달하지 못하는 이유

| 원인 | 설명 |
|------|------|
| **IP·도메인 평판 부재** | 새로 만든 도메인은 ISP에서 '신뢰할 수 없는 발신자'로 분류되어 차단율이 높습니다. |
| **보안 인증 설정의 복잡성** | 이메일 조작을 막는 3대 핵심 인증(SPF, DKIM, DMARC)을 DNS에 완벽하게 설정하지 않으면 메일이 즉시 반송됩니다. |
| **공유 IP 오염** | 저렴한 웹호스팅이나 클라우드의 공용 IP를 사용하면, 같은 IP를 쓰던 다른 사용자의 스팸 이력으로 내 메일까지 차단됩니다. |
| **대량 발송 통제 실패** | 갑자기 수백 명의 가입자에게 인증 메일을 동시에 보내면 ISP는 이를 스팸 공격으로 인지해 도메인을 차단합니다. |

### Resend가 높은 수신율을 보장하는 방법

Resend는 위 기술적 문제를 백엔드에서 자동으로 해결하여 메일이 수신자의 기본 수신함에 안전하게 전달되도록 합니다.

```
[내 애플리케이션] ──(API 호출)──▶ [Resend의 신뢰된 서버] ──(최적화 발송)──▶ [사용자 수신함]
                                              │
                               ┌──────────────┴──────────────┐
                               ▼                             ▼
                    [보안 인증 자동화]             [전용 IP 및 평판 관리]
                (SPF / DKIM / DMARC)           (대형 ISP 화이트리스트)
```

| 기능 | 설명 |
|------|------|
| **보안 인증 가이드 및 검증** | 도메인 등록 시 SPF, DKIM, DMARC 레코드 값을 정확히 지정하고 실시간으로 검증합니다. |
| **철저히 관리된 발송 IP** | 수신율이 검증된 깨끗한 IP 풀(Pool)을 통해서만 발송하며, 스팸 발송자를 퇴출하여 IP 평판을 유지합니다. |
| **IP 워밍업 자동화** | 발송량을 점진적으로 늘려 대형 ISP(Gmail 등)가 도메인을 안전한 발신자로 인식하도록 대신 처리합니다. |
| **ISP 가이드라인 실시간 준수** | 원클릭 구독 취소(List-Unsubscribe) 및 발송 속도 제한 규정을 API 내부적으로 자동 준수합니다. |

### 자체 운영 vs Resend 비교

| 평가 항목 | 자체 도메인 서버 직접 운영 | Resend 도입 |
|----------|--------------------------|------------|
| 초기 수신율 | 매우 낮음 (대부분 스팸함) | 매우 높음 (기본 수신함 안착) |
| 구축 시간 | 수일~수주 (리눅스 설정 및 DNS 작업) | 5분 이내 (API 연동 및 DNS 인증) |
| 보안 인증 관리 | SPF, DKIM, DMARC 수동 관리 및 갱신 | 발급된 값 복사·붙여넣기로 자동 해결 |
| 모니터링 | 반송(Bounce) 원인 추적 불가 | 실시간 대시보드로 도달 실패 사유 제공 |
