...
기본 웹사이트(정적 페이지)를 넘어서, 다음과 같은 기능이 필요해지면  
각각의 서비스가 추가됩니다.
...
2. 기본 코드가 편집창에 표시됩니다 → 다음 단계에서 실제 코드로 교체
3. **[Deploy]** 클릭 (일단 저장)

### 6.2 DART 전용 Worker 코드 입력

> 다음 코드는 브라우저 대신 DART API에 접속해서 데이터를 받아오는 전체 코드입니다.  
> 코드 내용을 이해하지 못해도 됩니다. 그대로 복사해서 붙여넣으면 됩니다.

**코드 편집 메뉴 경로**:  
`Workers & Pages → dart-proxy 클릭 → [Edit Code] 버튼`

기존 코드를 전부 지우고 다음 코드를 붙여넣은 후 **[Deploy]**:
...
다음 항목을 순서대로 실행합니다. **자신의 카드로 실제 결제**합니다.
...
다음 명령은 모두 `synology-web/` 폴더에서 실행합니다.
