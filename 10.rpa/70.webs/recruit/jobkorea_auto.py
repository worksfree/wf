...
async def ensure_logged_in(page) -> bool:
    ...
    # 이미 로그인 상태 확인 — 사용자 버튼에 "남산HR" 텍스트가 있으면 로그인된 것
    try:
        user_btn = page.locator('xpath=//*[@id="base-ui-_R_t99kltlb_"]/button/span/span')
        if await user_btn.is_visible(timeout=2000):
            btn_text = (await user_btn.inner_text(timeout=1000)).strip()
            if btn_text == '남산HR':
                print('[로그인] 기존 세션 확인 (남산HR) → 인재검색 클릭')
                await page.click('text=인재검색', timeout=5000)
                await asyncio.sleep(1.5)
                await close_popups(page)
                return True
    except Exception:
