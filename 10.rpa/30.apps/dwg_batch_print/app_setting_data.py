# -*- coding: utf-8 -*-
"""
DWG Batch Print Configuration Loader
...
class Config:
    ...
    def __init__(self):
        ...
        # 통합 사용자 설정 파일 (배포: ~/.wf_rpa/dwg_batch_print/settings.json)
        ...
        if self.run_mode in ("dev", "demo"):
            ...
            base = Path.home() / ".wf_rpa" / "DWG_Batch_Print"
    ...
    def logger(self):
        ...
        if self._logger is None:
            self._logger = wflog.get_app_logger("DWG_Batch_Print", console_level=logging.DEBUG)
        return self._logger
    ...
    def _ensure_settings_file(self):
        ...
        if not self.settings_file.exists():
            ...
            is_frozen = getattr(sys, 'frozen', False)
            if is_frozen:
                bundled_config = Path(self.base_dir) / "config" / "dwg_batch_print" / "settings.json"
    ...
    def _load_app_policy_cached(self):
        ...
        try:
            ...
            with open(policy_file, "r", encoding="utf-8") as f:
                ...
                # 직접 정책 데이터를 읽음 (policies.dwg_batch_print 구조가 아님)
    ...
    def _load_app_policy(self):
        """크레딧 정책 파일에서 dwg_batch_print 설정 로드 (credit_policy.json)"""
    ...
    def _apply_config(self, config_data):
        ...
        # 앱 정보
        self.app_name = runtime_config.get("app_name", "dwg_batch_print")
        self.display_name = runtime_config.get("display_name", "DWG 일괄 인쇄")
