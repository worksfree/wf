...
def load_and_increment_version():
    ...
    settings_file = COMMON_CONFIG_LOCAL / "dwg_batch_print" / "settings.json"
...
APP_DISPLAY_NAME = "DWG Batch Print"
APP_DESCRIPTION = "DWG 파일 일괄 인쇄 도구"
...
def collect_essential_resources():
    ...
    essential_modules = [
        ...
        'wf_app_init_helpers.py', 'wf_hwinfo.py', 'wf_settings_common.py',
        'wf_googlesheets_manager.py', 'wf_email.py',
    ]
    ...
    # settings.json을 .wf_rpa/dwg_batch_print/ 위치로 번들링
    settings_src = COMMON_CONFIG_DIR / "dwg_batch_print" / "settings.json"
    if settings_src.exists():
        datas.append((str(settings_src), ".wf_rpa/dwg_batch_print"))
        print(f"설정 파일 번들: settings.json → .wf_rpa/dwg_batch_print/")

    # policy.json은 identity + policy만 포함 (변하지 않는 값들)
    policy_src = COMMON_CONFIG_DIR / "dwg_batch_print" / "policy.json"
    if policy_src.exists():
        datas.append((str(policy_src), ".wf_rpa/dwg_batch_print"))
        print(f"✓ policy.json 번들링됨 (.wf_rpa/dwg_batch_print/)")
...
def collect_missing_libraries():
    ...
    app_libs = ['keyboard', 'pyautogui', 'pywinauto']
...
exe = EXE(
    ...
    # icon=["res\\dp.ico"],  # Old icon (16x16 only)
    icon=["res\\02_DWG_Batch_Print.ico"],  # New icon (16, 32, 48, 256)
)
coll = COLLECT(
    exe,
    a.binaries,
...
def prepare_user_configs():
    ...
    if settings_src.exists():
        ...
        # 저장
        app_settings_dir = wf_rpa_dir / "dwg_batch_print"
        ...
        print(f"✓ settings.json 처리 완료 (.wf_rpa/dwg_batch_print/) - 버전: {APP_VERSION_FULL}, 빌드: #{VERSION_INFO['build_count']}")
    else:
    ...
    if policy_src.exists():
        ...
        set_hidden_attribute(target_file)
        print(f"✓ policy.json 복사 완료 (.wf_rpa/dwg_batch_print/)")
...
def post_build_automation():
            ...
            if result.returncode == 0:
                ...
                if installer_file.exists():
                    ...
                    if final_installer.exists():
                        final_installer.unlink()  # 기존 파일 삭제
    ...
    try:
        ...
        # NOTE: 이전 버전에서 중괄호를 이중으로 사용하여 set({dict}) 형태가 되어
        # TypeError: unhashable type: 'dict' 예외가 발생함.
        # 올바르게 단일 dict 리터럴로 수정.
        build_info = {
            "app_name": APP_NAME,
