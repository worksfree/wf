...
1. 폴더 구조 확인:
   ...
   ├── ui_main.py
   ├── requirements.txt
   ├── config/dwg_batch_print/settings.json
   ├── logs/
   └── demo_captures/
   ```
...
config = get_config()
automation = DwgBatchPrintAutomation(folder_path="D:/path/to/dwg_folder", console_mode=True)
...
- 배포 모드: `~/.wf_rpa/DWG_Batch_Print/logs/` 아래 로그
