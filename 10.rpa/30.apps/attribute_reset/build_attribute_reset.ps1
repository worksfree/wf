...
$AppName = 'attribute_reset'
$AppDisplayName = 'Attribute Reset'
$AppDir = $ScriptDir
$SpecFile = Join-Path $AppDir 'attribute_reset.spec'
$Python = 'python'
...
Write-Host "==== ${AppName} 패키징(Build Option=$BuildType) 시작 ====" -ForegroundColor Cyan
Ensure-Dir $Candidates

# 1) 빌드 (onedir 필요 여부)
$needOnedir = $true
switch($BuildType){ 1 { } 2 { } 3 { } 4 { } 5 { } default { $BuildType = 1 } }
...
foreach ($vp in $versionPaths) {
    if (Test-Path $vp) {
        ...
        if ($version) {
            Write-Host "   버전 확인: $version (from $vp)" -ForegroundColor Gray
            break
        }
    }
}
if(-not $version){ $version = '0.0.0.0'; Write-Host "   버전 정보 없음, 기본값 사용: $version" -ForegroundColor Yellow }
...
switch($BuildType){
  1 {
    ...
    if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName $AppDisplayName -Version $version }
  }
  2 {
    $paths = Copy-Portable -src $distPath -targetRoot $Candidates -version $version
