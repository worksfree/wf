# Batch Print 빌드 스크립트
# PyInstaller를 사용하여 batch_print.spec 파일로 빌드
...
$AppName = 'batch_print'
$AppDir = 'D:\drive_files\10.worksfree\10.rpa\30.apps\batch_print'
$script:VerifyScript = 'D:\drive_files\10.worksfree\10.rpa\scripts\verify_package_integrity.ps1'
$script:ReleasePath = $null
$SpecFile = 'D:\drive_files\10.worksfree\10.rpa\30.apps\batch_print\batch_print.spec'
$Python = 'python'
    ...
    try {
        ...
        if (Test-Path $shortcutPath) {
            try {
                ...
                if ($oldDesc) { Write-Host "   이전 정보: $oldDesc" -ForegroundColor Gray }
                Write-Host "   → 새 버전으로 대체합니다 (v$Version)" -ForegroundColor Cyan
            } catch { Write-Host "⚠️  기존 바로가기 발견 (정보 읽기 실패)" -ForegroundColor Yellow }
        }
...
try {
    # 1. onedir 빌드
    Write-Host "==> PyInstaller onedir 빌드 중..." -ForegroundColor Yellow
    ...
    switch($BuildType){
      1 { 
        ...
        if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "DWG Batch Print" -Version $version }
      }
      2 { 
        $paths = Copy-Portable -src $distDir -targetRoot $Candidates -version $version
        $null = Make-Zip $paths.Portable
        Write-Host "✓ Mode 2: onedir + ZIP 생성" -ForegroundColor Green
        $exePath = Join-Path $paths.Portable "$AppName.exe"
        if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "DWG Batch Print" -Version $version }
      }
      3 { 
        ...
        Write-Host "✓ Mode 3: onedir + ZIP + Installer 생성" -ForegroundColor Green
        $exePath = Join-Path $paths.Portable "$AppName.exe"
        if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "DWG Batch Print" -Version $version }
      }
      4 { $tmp = Copy-Portable -src $distDir -targetRoot $Candidates -version $version; $paths = @{ Base=$tmp.Base; Portable=$null }; $zip = Make-Zip $tmp.Portable; Remove-Item -Recurse -Force $tmp.Portable; Write-Host "✓ Mode 4: ZIP만 생성" -ForegroundColor Green }
      5 { $tmp = Copy-Portable -src $distDir -targetRoot $Candidates -version $version; $paths = @{ Base=$tmp.Base; Portable=$null }; $exe = Make-Installer $tmp.Portable; if($tmp.Portable -and (Test-Path $tmp.Portable)){ Remove-Item -Recurse -Force $tmp.Portable }; Write-Host "✓ Mode 5: Installer만 생성" -ForegroundColor Green }
    ...
    if ($Clean.IsPresent) {
        ...
        Clean-Tree "$AppDir\build"
        Clean-Tree "$AppDir\dist"
        Get-ChildItem -Path $AppDir -Filter "*.exe" -File | Remove-Item -Force -ErrorAction SilentlyContinue
    ...
    if ($PostClean.IsPresent) {
        ...
        if (Test-Path $cleanup) { 
            Write-Host "==> 루트 정리 실행" -ForegroundColor Yellow
            & $cleanup 
        }
    }
