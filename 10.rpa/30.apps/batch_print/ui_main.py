...
def _get_startup_log_path():
    ...
    if mode in ("dev", "demo"):
        ...
        path = Path.home() / ".wf_rpa" / "batch_print" / "startup_profile.log"
    try:
...
# ==================== SINGLE INSTANCE & CROSS-APP LOCK ====================
def _acquire_single_instance(mutex_name: str = r"Global\\WF_BP"):
...
def _load_version_info():
    ...
    try:
        if getattr(sys, "frozen", False):
            ...
            settings_file = base_path / ".wf_rpa" / "batch_print" / "settings.json"

            # fallback: 사용자 홈 (버전 정보가 없을 수 있음)
            if not settings_file.exists():
                settings_file = Path.home() / ".wf_rpa" / "batch_print" / "settings.json"
        else:
            # 개발 모드: 10.common/config/batch_print/settings.json (통합 경로)
            app_root = Path(__file__).parent
            settings_file = app_root.parent.parent / "10.common" / "config" / "batch_print" / "settings.json"
            # fallback: 앱 폴더의 config
            if not settings_file.exists():
                settings_file = app_root / "config" / "batch_print" / "settings.json"
...
# BatchPrintAutomation은 lazy import로 변경 (필요 시점에 로드하여 startup 시간 단축)
# from automation import BatchPrintAutomation
_log_startup("automation.BatchPrintAutomation deferred (lazy import)")
...
try:
    ...
    def get_app_logger(name: str, console_level: int = logging.DEBUG):  # type: ignore
...
class BatchPrintGUIApplication:
    """Batch Print GUI 애플리케이션 클래스 (bom_exporter의 be 스키마 완벽 동일)"""
    ...
    def __init__(self, master):
        ...
        self.logger = get_app_logger("batch_print", console_level=logging.DEBUG)
        ...
        self.automation: Any = None  # BatchPrintAutomation (lazy import)
        ...
        try:
            ...
            pass
        _log_startup("BatchPrintGUIApplication initialized - UI ready")
        _flush_startup_log()
    ...
    def _find_icon_path(self):
        ...
        try:
            icon_names = ["02_Batch_Print.ico", "bp.ico"]
            if getattr(sys, 'frozen', False):
                base_paths = [
    ...
    def check_and_set_execution_status(self):
        ...
        try:
            ...
            exec_status["current_app"] = "batch_print"
    ...
    def init_test_server(self):
        ...
        try:
            ...
            self.test_server = TestServer(app_name="batch_print")
            ...
            self.test_server.register_handlers({
                ...
                'check_credentials': self._test_check_credentials,
                'test_google_sheets_connectivity': self._test_google_sheets_connectivity,
                'check_credentials': self._test_check_credentials,
                'test_google_sheets_connectivity': self._test_google_sheets_connectivity,
    ...
    def _test_get_policy(self) -> dict:
        ...
        return {
            ...
            "policy": {
                ...
                "trial_credits": policy_data.get("trial_credits", 20000),
    ...
    def _test_get_trial_info(self) -> dict:
        ...
        try:
            ...
            data = self.credit_manager._load_credit_data()
            return {
                "initial_credits": policy.get("trial_credits", 20000),
                "remaining_trial": data.get("remaining_trial", 0),
                "trial_credits": policy.get("trial_credits", 20000),
            }
        except Exception as e:
            return {"error": str(e)}
    ...
    def _test_save_settings(self, settings: dict = None, **kwargs) -> dict:
        """설정 저장 (테스트용)"""
        try:
            import json
            settings_file = Path.home() / ".wf_rpa" / "batch_print" / "settings.json"
            ...
            data["test_config"].update(payload)
    ...
    def _test_check_credentials(self) -> dict:
        """크리덴셜 파일 존재 확인 (테스트용)"""
        try:
            import json
            config_path = Path(__file__).resolve().parents[2] / "10.common" / "config" / "wf_rpa_config.json"
            if not config_path.exists():
                return {"available": False, "error": f"config_not_found: {config_path}"}

            with open(config_path, "r", encoding="utf-8") as f:
                config = json.load(f) or {}

            gs = config.get("google_sheets", {})
            cred_file = (gs.get("credentials_file_dev") or gs.get("credentials_file_release") or "").strip()
            if not cred_file:
                return {"available": False, "error": "credentials_file_not_configured"}

            cred_path = config_path.parent / cred_file
            return {"available": cred_path.exists(), "path": str(cred_path)}
        except Exception as e:
            return {"available": False, "error": str(e)}

    def _test_google_sheets_connectivity(self) -> dict:
        """Google Sheets 연결 테스트 (테스트용)"""
        try:
            import json
            config_path = Path(__file__).resolve().parents[2] / "10.common" / "config" / "wf_rpa_config.json"
            if not config_path.exists():
                return {"connected": True, "skipped": True, "error": "config_not_found"}

            with open(config_path, "r", encoding="utf-8") as f:
                config = json.load(f) or {}

            gs = config.get("google_sheets", {})
            sheet_id = (gs.get("sheet_id_dev") or gs.get("sheet_id_release") or "").strip()
            cred_file = (gs.get("credentials_file_dev") or gs.get("credentials_file_release") or "").strip()
            if not sheet_id or not cred_file:
                return {"connected": True, "skipped": True, "error": "not_configured"}

            cred_path = config_path.parent / cred_file
            if not cred_path.exists():
                return {"connected": False, "error": f"credentials_not_found: {cred_path}"}
            ...
            try:
                ...
                return {"connected": bool(spreadsheet.title)}
            except ImportError:
                return {"connected": True, "skipped": True, "error": "gspread_not_installed"}
        except Exception as e:
    ...
    def create_user_directories(self):
        ...
        app_dir = os.path.join(wf_rpa_dir, "batch_print")
    ...
    def on_refresh_credit(self):
        ...
        try:
            ...
            try:
                from wf_settings_common import sync_policies_from_sheets  # type: ignore
                policy_result = sync_policies_from_sheets("batch_print", self.logger)
    ...
    def select_folder_license_check(self):
        ...
        # Lazy import: BatchPrintAutomation은 폴더 선택 시점에 로드 (startup 시간 최적화)
        try:
            self.logger.info(f"폴더 선택됨: {self.DWG_PATH}")
            self.logger.debug("BatchPrintAutomation 모듈 로딩 시작...")
            _log_startup("Loading BatchPrintAutomation (lazy)")
            from automation import BatchPrintAutomation
            self.logger.debug("BatchPrintAutomation 모듈 로딩 완료")
            _log_startup("BatchPrintAutomation loaded")

            self.logger.debug("BatchPrintAutomation 인스턴스 생성 시작...")
            self.automation = BatchPrintAutomation(folder_path=self.DWG_PATH, console_mode=False)
            self.logger.debug("BatchPrintAutomation 인스턴스 생성 완료")
        ...
        # 설정 적용
            ...
            self.logger.error(f"BatchPrintAutomation 초기화 실패:\n{error_details}")
            messagebox.showerror(
                "초기화 실패",
    ...
    def start_dwg_print(self):
        ...
        # BatchPrintAutomation 객체가 없으면 여기서 생성 (실제 작업 시점)
        if not hasattr(self, "automation") or self.automation is None:
            try:
                self.logger.info("BatchPrintAutomation 객체 생성 중...")
                from automation import BatchPrintAutomation

                self.automation = BatchPrintAutomation(
                ...
                self.automation.set_credit_update_callback(self.update_credit_display)
                self.automation.set_credit_manager(self.credit_manager)
                self.logger.info("BatchPrintAutomation 객체 생성 완료")
            except Exception as e:
                self.logger.error(f"BatchPrintAutomation 초기화 실패: {e}")
...
def _handle_sync_registration():
    ...
    try:
        ...
        result = manager.sync_local_registration_to_sheets("batch_print", app_version)
...
def main():
    ...
    if not test_mode:
        ...
        if not is_first:
            ...
            try:
                print("Another batch_print instance is already running. Exiting.")
            except Exception:
                pass
            return
        ...
        if check_cross_app_running_and_exit:
            check_cross_app_running_and_exit("batch_print")
    else:
    ...
    try:
        _set_cross_app_running("batch_print")
    ...
    try:
        # 아이콘 파일명 (새 아이콘: 02_Batch_Print.ico, 기존: bp.ico)
        icon_names = ["02_Batch_Print.ico", "bp.ico"]
    ...
    try:
        target_res = (
            ...
            else Path.home() / ".wf_rpa" / "batch_print" / "res"
        )
        bundle_candidates = [
            Path(__file__).parent / "res",
        ]
    ...
    app = BatchPrintGUIApplication(root)
    _log_startup("BatchPrintGUIApplication initialized - UI ready")
    _flush_startup_log()

    # WF-ACT 테스트 모드: TestServer 초기화 (여기서만 초기화, 아래 중복 제거)
    if test_mode:
        try:
    ...
    try:
        ...
        if _run_mode == "demo":
            try:
                ...
                h = app.ui.get("window_height", 200)
