...
def _detect_run_mode():
    settings_path = Path(__file__).parent / "config" / "bom_exporter" / "settings.json"
    try:
        with open(settings_path, "r", encoding="utf-8") as f:
            data = json.load(f) or {}
        cfg_mode = str(data.get("app_config", {}).get("run_mode", "") or "").strip().lower()
...
def _get_startup_log_path():
    ...
    if mode in ("dev", "demo"):
        ...
        path = Path.home() / ".wf_rpa" / "bom_exporter" / "startup_profile.log"
    try:
...
def _log_startup(msg):
    """Startup 타이밍 로그 수집 - 즉시 파일에 기록"""
    if not _STARTUP_ENABLED:
        return
    import time

    t = time.perf_counter()
    _STARTUP_LOG.append((t, msg))
    # 즉시 파일에 기록 (버퍼링 없이)
    try:
        log_path = _get_startup_log_path()
        with open(log_path, "a", encoding="utf-8") as f:
            if len(_STARTUP_LOG) == 1:
                f.write(f"\n{'='*60}\n")
                f.write("STARTUP PROFILING (Real-time)\n")
                f.write(f"{'='*60}\n")
...
def _flush_startup_log():
    ...
    try:
        ...
        with open(log_path, "w", encoding="utf-8") as f:
            f.write("=" * 60 + "\n")
            f.write("STARTUP PROFILING REPORT\n")
            f.write("=" * 60 + "\n")
...
def _load_version_info():
    ...
    try:
        if getattr(sys, "frozen", False):
            # 릴리스 모드: 번들된 settings.json을 우선 사용 (.wf_rpa/bom_exporter/settings.json)
            ...
            # 개발 모드: config/bom_exporter/settings.json
...
def _acquire_single_instance(mutex_name: str = r"Global\\WF_BOM_EXPORTER"):
...
# -*- coding: utf-8 -*-
"""
Bom Exporter Main UI Module
메인 GUI 인터페이스를 담당하는 모듈
"""

# --- Windows DPI awareness: set process-level DPI before any GUI init ---
# This helps Tkinter report correct logical DPI on UHD/High-DPI displays.
...
# BomAutomation은 lazy import로 변경 (필요 시점에 로드하여 startup 시간 단축)
# from automation import BomAutomation
_log_startup("automation.BomAutomation deferred (lazy import)")
from ui_setting import create_settings_window, load_custom_settings, apply_custom_settings_to_config, _apply_global_fonts
...
class BomGUIApplication:
    """Bom Exporter GUI 애플리케이션 클래스"""
    ...
    def __init__(self, master):
        ...
        # 버전 정보 저장 (설정창에서 사용)
        self.APP_VERSION_FULL = APP_VERSION_FULL
        self.APP_VERSION_DISPLAY = APP_VERSION_DISPLAY
        ...
        # 🚀 최적화: 다중 실행 체크를 나중으로 지연 (200ms 절약)
        # if not self.check_and_set_execution_status():
        #     self.logger.error("다른 워크스프리 앱이 실행 중입니다. 한 번에 하나의 앱만 실행할 수 있습니다.")
        # MessageBox들이 메인창 기준으로 뜨도록 parent를 강제 지정
        self._bind_messagebox_parent()
        #     messagebox.showerror("실행 제한", "다른 워크스프리 앱이 실행 중입니다.\n한 번에 하나의 앱만 실행할 수 있습니다.")
        #     sys.exit(0)
        ...
        _log_startup(f"Geometry override initialized: {self.debug_geometry_override}")
        ...
        self.SLDDRW_PATH = None  # 하위 호환성 유지 (임시)
        ...
        self.already_completed_count = 0  # 재실행 시 기존 완료 개수
        ...
        self.admin_password = "admin2024"  # 관리자 비밀번호 (실제 환경에서는 암호화 필요)
        ...
        try:
            if self._wfm_available and WorksFreeManager:
                ...
                self.logger.debug(f"[PERF] Early WorksFreeManager init: {(t1-t0)*1000:.2f} ms, registered={self.is_registered_user}")
        except Exception as e:
        ...
        try:
            ...
            pass
        _log_startup("BomGUIApplication initialized - UI ready")
        _flush_startup_log()
    ...
    def set_selected_path(self, path: str | None):
        ...
        self.SLDDRW_PATH = path  # 하위 호환 유지
    ...
    def _start_global_hotkey_listener(self):
        ...
        try:
            # 데모 모드에서만 활성화
            if not self.demo_capture_enabled:
                return
        except Exception:
            return
        ...
        try:
            ...
            def _thread_main():
                ...
                if not user32.RegisterHotKey(None, HOTKEY_ID, MOD_ALT, VK_C):
                    try:
                        if self.logger:
                            self.logger.debug(f"RegisterHotKey 실패: 이미 다른 앱에서 사용 중일 수 있음")
                    except Exception:
                        pass
                    return
                ...
                try:
                    ...
                    while True:
                        ...
                        if msg.message == WM_HOTKEY and msg.wParam == HOTKEY_ID:
                            try:
                                # Tk 메인스레드에서 안전하게 호출
                                self.master.after(0, self._on_manual_capture)
                            except Exception:
                                pass
                        # 일반 메시지는 무시
                finally:
    ...
    def _on_manual_capture(self, _event=None):
        ...
        if not self.demo_capture_dir:
            self._show_toast("캡처 폴더가 설정되지 않았습니다", duration_ms=1500)
            return
    ...
    def check_and_set_execution_status(self):
        ...
        try:
            ...
            exec_status["current_app"] = "bom_exporter"
    ...
    def clear_execution_status(self):
        ...
        try:
            ...
            home = Path.home()
            config_file = home / ".wf_rpa" / "wf_rpa_config.json"
    ...
    def on_closing(self):
        ...
        # 앱 종료 시: 크레딧 동기화를 논블로킹으로 처리
        # (사용자가 닫기 버튼을 눌렀을 때 즉시 종료되도록 함)
        try:
            if getattr(self, "credit_manager", None):
                status = self.credit_manager.get_sync_status()
                if status.get("needs_sync"):
                    self.logger.info("🔄 앱 종료 시 크레딧 동기화 예약 (백그라운드)...")
                    # 동기화는 다음 실행 시 처리됨
        except Exception as e:
            self.logger.warning(f"[SYNC-EXIT] 동기화 상태 확인 중 오류: {e}")
    ...
    def create_user_directories(self):
        ...
        app_dir = os.path.join(wf_rpa_dir, "bom_exporter")
    ...
    def init_ui(self):
        ...
        # Saved/demo geometry override (env > settings) - align with other apps
        ...
        if self.logger:
            self.logger.debug(f"[FONT] self.ui['font_size'] = {self.ui.get('font_size', 'N/A')}")
    ...
    def create_ui_elements(self):
        ...
        self.print_button = tk.Button(
            button_frame,
            text="BOM 저장",
            command=self.start_bom_extraction,
            width=12,
            height=1,
            state="disabled",
            font=("맑은 고딕", self.ui["font_size"]),
    ...
    def on_refresh_credit(self):
        ...
        try:
                ...
                if result.get("success"):
                    ...
                    if added > 0:
                        ...
                        for tid in applied_ids:
                            # timestamp 형식이면 더 읽기 쉽게 표시
                            if "T" in tid and ":" in tid:
                                display_id = (
                                    tid.replace("T", " ").replace(":", "시").replace(".", "분")
                                )
                            else:
                                display_id = tid
                            msg += f"• {display_id}\n"
            ...
            # 
            #     policy_result = sync_policies_from_sheets("bom_exporter", self.logger)
            # 
    ...
    def _enter_admin_mode(self):
        ...
        # UI 변경
        self.master.title(f"BOM 엑셀 저장 {APP_VERSION_FULL} [🔧 관리자 모드]")
        ...
        # 관리자 모드 활성화 로그 (기존 로거 사용)
        import ctypes

        try:
            is_admin = ctypes.windll.shell32.IsUserAnAdmin()
            print(f"관리자 권한으로 실행 중: {is_admin}")  # 콘솔과 UI에 동시 출력
        except Exception:
            print("관리자 권한 확인 실패")
    ...
    def on_scan_toggle(self):
        ...
        if self.scan_toggle_var.get():
            ...
            try:
                ...
                # 경로 유효성 재확인
                if not self.SLDDRW_PATH:
                    self.logger.error("SLDDRW_PATH가 None입니다")
                    messagebox.showerror("오류", "폴더 경로가 설정되지 않았습니다.")
                    self.scan_toggle_var.set(False)
                    return

                # CV 방식: 간단하게 파일 카운트만 수행 (BomAutomation 생성 지연 - 배포 환경 안정성)
                from pathlib import Path

                self.logger.debug(f"SLDDRW_PATH: {self.SLDDRW_PATH}")
                folder_path = Path(self.SLDDRW_PATH)
                
                if not folder_path.exists():
                    self.logger.error(f"폴더가 존재하지 않습니다: {self.SLDDRW_PATH}")
                ...
                if pending_path.exists():
                    try:
                        ...
                        self.logger.info(f"재개 가능한 목록 감지: {len(pending_files)}개")
                    except Exception as e:
                        self.logger.warning(f"pending 목록 읽기 실패: {e}")
                ...
                if pending_files:
                    ...
                    if file_count == 0:
                        ...
                        file_count = len(fallback_matches)
                        if file_count > 0:
                            self.logger.info(
                                "기본 규칙 일치 파일이 없어 '-00.slddrw' 종단 규칙으로 대체합니다."
                            )
    ...
    def select_folder_license_check(self):
        ...
        self.folder_entry.config(state="normal")
        self.folder_entry.delete(0, tk.END)
        self.folder_entry.insert(0, self.SLDDRW_PATH)
        self.folder_entry.config(state="readonly")
        
        # 폴더 선택 캡처

        # Lazy import: BomAutomation은 폴더 선택 시점에 로드 (startup 시간 최적화)
        try:
            self.logger.info(f"폴더 선택됨: {self.SLDDRW_PATH}")
            self.logger.debug("BomAutomation 모듈 로딩 시작...")
            _log_startup("Loading BomAutomation (lazy)")
            from automation import BomAutomation
            self.logger.debug("BomAutomation 모듈 로딩 완료")
            _log_startup("BomAutomation loaded")

            self.logger.debug("BomAutomation 인스턴스 생성 시작...")
            self.automation = BomAutomation(
                folder_path=self.SLDDRW_PATH, 
                console_mode=False,
                ignore_processed_for_rerun=getattr(self, "rerun_ignore_processed", False),
                only_count_new_duplicates=getattr(self, "rerun_only_new_duplicates", False)
            )
            self.logger.debug("BomAutomation 인스턴스 생성 완료")
            ...
            self.automation.set_capture_callback(self._capture_demo)
            self.logger.debug("콜백 함수 설정 완료")
        except Exception as e:
            import traceback
            error_details = traceback.format_exc()
            self.logger.error(f"BomAutomation 초기화 실패:\n{error_details}")
            messagebox.showerror(
                "초기화 실패",
        ...
        # 스캔은 automation에 위임하여 pending 목록을 우선 반영
        try:
            ...
            self.logger.debug(f"process_folder 호출: {self.SLDDRW_PATH}")
            self.automation.process_folder(self.SLDDRW_PATH, scan_only=True)
            self.logger.debug("process_folder 완료")
            
            # ===== 재실행 감지 로직 (수정 버전) =====
            # total_count: 처리할 파일 개수 (남은 파일만)
            # total_count_original: 전체 파일 개수 (이미 처리된 파일 포함)
            total_files = getattr(self.automation, "total_count_original", 0)  # 전체 파일
            remaining_files = getattr(self.automation, "total_count", 0)  # 남은 파일
            processed_count = len(getattr(self.automation, "processed_files", set()))  # 이미 처리된 파일
            
            self.logger.debug(f"스캔 결과: 전체 {total_files}개, 남은 {remaining_files}개, 처리완료 {processed_count}개")
            ...
            if total_files > 0:
                ...
                # exported_bom 폴더의 실제 파일 개수
                exported_count = self.automation.count_exported_files()
                has_pending = self.automation.has_credit_shortage_interruption()
                
                run_mode = str(getattr(getattr(self.automation, "config", None), "run_mode", "release")).lower()
                is_demo_mode = run_mode == "demo"
                
                self.logger.debug(
                    f"재실행 상태: exported={exported_count}, processed={processed_count}, "
                    f"total={total_files}, remaining={remaining_files}, pending={has_pending}"
                )
                
                # ===== 케이스 1: 완전 신규 실행 =====
                # exported_bom 폴더도 없고, processed_files도 없음
                if exported_count == 0 and processed_count == 0:
                    self.logger.info("✅ 신규 실행: 기존 결과나 처리 기록이 없습니다.")
                
                # ===== 케이스 2: 크레딧 부족 재개 =====
                # wf_pending_list.txt가 존재하고, 남은 파일이 있음
                elif has_pending and remaining_files > 0:
                    response = messagebox.askyesno(
                        "작업 재개",
                        f"크레딧 부족으로 중단된 작업입니다.\n\n"
                        f"• 전체 파일: {total_files}개\n"
                        f"• 완료된 파일: {exported_count}개\n"
                        f"• 남은 파일: {remaining_files}개\n\n"
                        f"남은 작업을 계속하시겠습니까?\n"
                        f"(완료된 파일은 건너뛰고 남은 파일만 처리합니다)"
                    )
                    if not response:
                        self.logger.info("사용자가 작업 재개를 취소했습니다.")
                        self.print_button.config(state="disabled")
                        self.progress_label.config(text="취소됨")
                        return
                    self.logger.info(f"✅ 크레딧 부족 재개: {exported_count}개 완료, {remaining_files}개 남음")
                
                # ===== 케이스 3: 완료된 폴더 재실행 =====
                # exported_count == total_files (모든 파일 처리 완료)
                # remaining_files == 0 (남은 파일 없음)
                elif exported_count >= total_files and remaining_files == 0:
                    # 크레딧 비용 계산 (policy.json에서 동적 읽기)
                    cost_per_work = 100  # 기본값
                    try:
                        if self.credit_manager:
                            cost_per_work = self.credit_manager.get_per_item_cost()
                    except Exception:
                        pass
                    total_cost = total_files * cost_per_work
                    
                    response = messagebox.askyesno(
                        "재실행 확인",
                        f"이미 작업이 완료된 폴더입니다.\n\n"
                        f"• 전체 파일: {total_files}개\n"
                        f"• 결과 폴더 파일: {exported_count}개\n\n"
                        f"⚠️ 중요: 다시 처리할 경우\n"
                        f"   - 기존 결과물을 모두 삭제합니다\n"
                        f"   - {total_files}개 파일을 모두 다시 처리합니다\n"
                        f"   - 크레딧 {total_cost}점이 차감됩니다\n"
                        f"     (중복 파일은 크레딧 차감 제외)\n\n"
                        f"계속하시겠습니까?"
                    )
                    if not response:
                        self.logger.info("사용자가 재실행을 취소했습니다.")
                        self.print_button.config(state="disabled")
                        self.progress_label.config(text="취소됨")
                        return
                    
                    # 기존 결과물 삭제 (덮어쓰기 문제 방지)
                    self.logger.info("✅ 완료된 폴더 재실행: 기존 결과물 삭제 시작")
                    try:
                        import shutil
                        from pathlib import Path
                        ...
                        # exported_bom 폴더 삭제
                        exported_bom_path = Path(folder_path) / "exported_bom"
                        if exported_bom_path.exists():
                            shutil.rmtree(exported_bom_path)
                            self.logger.info(f"삭제 완료: {exported_bom_path}")
                        
                        # BOM 폴더의 엑셀 파일 삭제
                        bom_path = Path(folder_path) / "BOM"
                        if bom_path.exists():
                            excel_files = list(bom_path.glob("*.xlsx")) + list(bom_path.glob("*.xls"))
                            for excel_file in excel_files:
                                excel_file.unlink()
                                self.logger.debug(f"삭제: {excel_file.name}")
                            self.logger.info(f"BOM 폴더 엑셀 파일 {len(excel_files)}개 삭제 완료")
                        
                        # wf_pending_list.txt 삭제 (크레딧 부족 기록 제거)
                        pending_path = Path(folder_path) / "wf_pending_list.txt"
                        if pending_path.exists():
                            pending_path.unlink()
                            self.logger.debug("wf_pending_list.txt 삭제")
                    except Exception as e:
                        self.logger.error(f"기존 결과물 삭제 실패: {e}")
                        messagebox.showerror("삭제 오류", f"기존 결과물 삭제 중 오류가 발생했습니다:\n{e}")
                        self.print_button.config(state="disabled")
                        return
                    
                    self.rerun_ignore_processed = True
                    self.rerun_only_new_duplicates = True
                    self.rerun_from_scratch = True  # 처음부터 재시작 플래그
                
                # ===== 케이스 4: 부분 완료 후 일반 재개 =====
                # pending 파일은 없지만 이미 처리된 파일이 있고 남은 파일도 있음
                elif processed_count > 0 and remaining_files > 0 and not has_pending:
                    # 크레딧 비용 계산
                    cost_per_work = 100  # 기본값
                    try:
                        if self.credit_manager:
                        ...
                        pass
                    estimated_cost = remaining_files * cost_per_work
                    
                    response = messagebox.askyesnocancel(
                        "작업 재개",
                        f"이전에 일부만 처리한 폴더입니다.\n\n"
                        f"• 전체 파일: {total_files}개\n"
                        f"• 이미 완료: {processed_count}개\n"
                        f"• 남은 파일: {remaining_files}개\n\n"
                        f"📌 선택 옵션:\n\n"
                        f"✅ 예: 남은 {remaining_files}개만 계속 처리\n"
                        f"   → 크레딧 {estimated_cost}점 차감\n"
                        f"   → 기존 결과 유지\n\n"
                        f"❌ 아니오: 처음부터 전체 다시 처리\n"
                        f"   → 크레딧 {total_files * cost_per_work}점 차감\n"
                        f"   → 기존 결과 삭제 후 재시작\n\n"
                        f"🚫 취소: 작업 중단\n\n"
                        f"어떻게 하시겠습니까?"
                    )
                    ...
                    if response is None:  # 취소
                        self.logger.info("사용자가 작업을 취소했습니다.")
                        self.print_button.config(state="disabled")
                        self.progress_label.config(text="취소됨")
                        return
                    elif response:  # 예: 계속 진행
                        self.logger.info(
                            f"✅ 부분 완료 재개: {processed_count}개 완료, {remaining_files}개 남음 "
                            f"(처리 완료된 파일은 자동으로 건너뜀)"
                        )
                    else:  # 아니오: 처음부터 재시작
                        self.logger.info("✅ 전체 재시작 선택: 기존 결과물 삭제 후 처음부터 재처리")
                        # 기존 결과물 삭제
                        try:
                            import shutil
                            from pathlib import Path
                        ...
                        self.rerun_ignore_processed = True
                        self.rerun_only_new_duplicates = True
                        self.rerun_from_scratch = True
                
                # ===== 케이스 5: 처리 기록 불일치 =====
                # exported_count와 processed_count가 불일치하는 경우
                elif exported_count != processed_count and not has_pending:
                    # 미처리 파일 계산 (결과 없는 파일만)
                    unprocessed_files = total_files - exported_count
                    cost_per_work = 100  # 기본값
                    try:
                        if self.credit_manager:
                            cost_per_work = self.credit_manager.get_per_item_cost()
                        ...
                        pass
                    estimated_cost = max(0, unprocessed_files) * cost_per_work
                    
                    response = messagebox.askyesno(
                        "불일치 감지",
                        f"처리 기록과 결과 파일이 일치하지 않습니다.\n\n"
                        f"• 전체 파일: {total_files}개\n"
                        f"• 결과 폴더 파일: {exported_count}개\n"
                        f"• 처리 기록: {processed_count}개\n\n"
                        f"⚠️ '예' 선택 시:\n"
                        f"   - 결과가 없는 파일만 처리 ({max(0, unprocessed_files)}개)\n"
                        f"   - 예상 크레딧 차감: {estimated_cost}점\n\n"
                        f"⚠️ '아니오' 선택 시:\n"
                        f"   - 작업을 취소합니다\n\n"
                        f"처음부터 다시 시작하시겠습니까?"
                    )
                    if not response:
                        ...
                        self.logger.info("✅ 처리 기록 불일치: 처음부터 재시작")
                        self.rerun_ignore_processed = True
                
                self.initial_file_count = total_files  # 전체 파일 개수로 초기화
                self.already_completed_count = processed_count  # 재실행 시 기존 완료 개수
                self.cumulative_processed_count = 0
                self.is_first_run = True
                self.last_run_success_count = 0
                self.progress_bar.config(maximum=self.initial_file_count, value=0)
                self.progress_label.config(text=f"0/{self.initial_file_count}")
                self.print_button.config(state="normal")
                self.logger.info(f"폴더 스캔 완료: {total_files}개 파일 발견")
            else:
                self.print_button.config(state="disabled")
                self.progress_label.config(text="파일 없음")
    ...
    def start_bom_extraction(self):
        """BOM 추출 시작 (백그라운드 스레드에서 실행)"""
        if not self.SLDDRW_PATH:
            return
        
        # BOM 추출 시작 캡처

        # BomAutomation 객체가 없으면 여기서 생성 (실제 작업 시점)
        if not hasattr(self, "automation") or self.automation is None:
            try:
                self.logger.info("BomAutomation 객체 생성 중...")
                from automation import BomAutomation
                ...
                self.automation.set_credit_manager(self.credit_manager)
                self.automation.set_capture_callback(self._capture_demo)
                self.logger.info("BomAutomation 객체 생성 완료")
            except Exception as e:
                self.logger.error(f"BomAutomation 초기화 실패: {e}")
                messagebox.showerror("초기화 실패", f"BOM 추출 준비 중 오류가 발생했습니다:\n{e}")
                return

        self.last_run_success_count = 0
        self.automation.process_folder(self.SLDDRW_PATH, scan_only=True)
        files_to_process_count = self.automation.total_count
        ...
        def worker():
            ...
            try:
                self.automation.open_sldprt_files()

            except Exception as e:
                exception_info["error"] = e
            finally:
                # 메인 스레드에서 UI 업데이트 (스레드 안전)
                self.master.after(0, lambda: self._on_extraction_complete(exception_info))

        # 스레드 시작
        threading.Thread(target=worker, daemon=True, name="BOM-Extraction-Worker").start()

    def _on_extraction_complete(self, exception_info):
        ...
        try:
            ...
            # ⚠️ 중요: 2차 시도까지 완료 후 최종 실패 파일 확인
            # automation.py에서 sldprt_files_missed에 2차 시도 실패 파일이 저장됨
            failed_files = getattr(self.automation, "sldprt_files_missed", [])

            if failed_files:
                self.progress_label.config(
                    text=f"{self.cumulative_processed_count}/{self.initial_file_count} ({len(failed_files)}개 실패)"
                )
                self.print_button.config(state="normal")
                ...
                if not getattr(self.automation, "credit_shortage_stop", False):
                    self.progress_label.config(
                        text=f"{self.cumulative_processed_count}/{self.initial_file_count} (완료)"
                    )
    ...
    def open_settings_window(self):
        ...
        if (
            settings_window_obj
            and hasattr(settings_window_obj, "settings_win")
            and settings_window_obj.settings_win
        ):
            toplevel_window = settings_window_obj.settings_win
...
def main():
    ...
    if not is_first:
        ...
        try:
            print("Another bom_exporter instance is already running. Exiting.")
        except Exception:
            pass
        return
    ...
    if check_cross_app_running_and_exit:
        check_cross_app_running_and_exit("bom_exporter")

    # Mark this app as running in the shared config (for cross-app guard)
    try:
        _set_cross_app_running("bom_exporter")
    ...
    # --- Temporary DPI handling for UHD experimentation ---
    # Enable Windows DPI awareness so Tkinter sees correct logical DPI
    if sys.platform == "win32":
        try:
            import ctypes
    ...
    # Temporary experiment: force internal Tk scaling to 1.0 (fonts only)
    try:
    ...
    try:
        if getattr(sys, 'frozen', False):
            # 릴리스: 실행 파일 인접 res 폴더
            icon_candidates = [
                Path(sys.executable).parent / "res" / "B2E.ico",
                Path(sys.executable).parent / "_internal" / "res" / "B2E.ico",
            ]
        else:
            # 개발: 앱 폴더 내 res 폴더
            icon_candidates = [Path(__file__).parent / "res" / "B2E.ico"]
    ...
    try:
        target_res = (
            ...
            else Path.home() / ".wf_rpa" / "bom_exporter" / "res"
        )
        bundle_candidates = [
            Path(__file__).parent / "res",
        ]
    ...
    try:
        ...
        # ===== 데모 캡처: Alt+C는 이제 RegisterHotKey (모달 호환) =====
        # 핫키 등록은 app.__init__에서 _init_demo_capture() 호출 시 처리됨
        # 기존 keyboard 라이브러리 방식은 제거 (RegisterHotKey가 더 견고함)
        # ===== 데모 캡처 끝 =====
        # 데모 모드: 오른쪽 2/3 영역의 중앙에 배치 (동적 계산)
        if _run_mode == "demo":
            try:
                # 데모 모드도 settings.json의 ui_config를 그대로 사용 (고정값 580x240 제거)
                w = app.ui.get("window_width", 580)
                h = app.ui.get("window_height", 380)
