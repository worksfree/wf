...
$AppName = 'dwg_classifier'
$AppDir = 'D:\drive_files\10.worksfree\10.rpa\50.data\dwg_classifier'
$SpecFile = 'D:\drive_files\10.worksfree\10.rpa\50.data\dwg_classifier\dwg_classifier.spec'
...
function Read-Version($settingsJson){ 
    if(!(Test-Path $settingsJson)){ 
        Write-Host "⚠ settings.json not found: $settingsJson" -ForegroundColor Yellow
        return $null 
    } 
    try{ 
        $content = Get-Content $settingsJson -Raw -Encoding UTF8
        $json = $content | ConvertFrom-Json
        $ver = $json.runtime_config.full_version
        if($ver){ 
            Write-Host "✓ Version read from settings: $ver" -ForegroundColor Green
            return $ver
        }
        Write-Host "⚠ full_version field is empty" -ForegroundColor Yellow
        return $null
    } catch { 
        Write-Host "⚠ Error reading version: $_" -ForegroundColor Yellow
        return $null 
    } 
}
function Build-Onedir{ Push-Location $AppDir; try{ $env:WF_EXTERNAL_PACKAGER='1'; $distPath = Join-Path $AppDir 'dist'; $buildPath = Join-Path $AppDir 'build'; if ((Test-Path $distPath) -or (Test-Path $buildPath)) { Get-Process | Where-Object {$_.Path -like "*$AppName*"} | Stop-Process -Force -ErrorAction SilentlyContinue; Start-Sleep -Seconds 1; for ($i = 0; $i -lt 3; $i++) { Remove-Item $distPath -Recurse -Force -ErrorAction SilentlyContinue; Remove-Item $buildPath -Recurse -Force -ErrorAction SilentlyContinue; if (!(Test-Path $distPath) -and !(Test-Path $buildPath)) { break }; Start-Sleep -Milliseconds 500 }; Start-Sleep -Seconds 1 }; Clean-Tree "$env:LOCALAPPDATA\pyinstaller"; $p=Start-Process -FilePath $Python -ArgumentList $PyInstallerArgs -Wait -PassThru -NoNewWindow; if($p.ExitCode -ne 0){ throw "PyInstaller failed ($($p.ExitCode))" }; if(!(Test-Path "$AppDir\dist\$AppName")){ throw 'dist missing' }; "$AppDir\dist\$AppName" } finally { Pop-Location } }
function Copy-Portable($src,$targetRoot,$version){ $base=Join-Path $targetRoot "${AppName}_${version}"; $portable=Join-Path $base "${AppName}_${version}_portable"; Ensure-Dir $base; if(Test-Path $portable){ Remove-Item -Recurse -Force $portable }; Copy-Item -Recurse -Force -LiteralPath $src -Destination $portable; $commonScriptSrc = Join-Path $PSScriptRoot '..\..\10.common\create_desktop_shortcut.bat'; if (Test-Path $commonScriptSrc) { $scriptDest = Join-Path $portable 'create_desktop_shortcut.bat'; Copy-Item -Path $commonScriptSrc -Destination $scriptDest -Force; Write-Host "✓ 바로가기 생성 스크립트 포함: create_desktop_shortcut.bat" -ForegroundColor Green }; @{ Base=$base; Portable=$portable } }
...
$dist = Build-Onedir
$ver = Read-Version (Join-Path $AppDir 'config\dwg_classifier\settings.json'); if(-not $ver){ $ver='v0.0.0.0' }
$paths = $null
switch($Option){
  1 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "DWG Classifier" -Version $ver } }
  2 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $null = Make-Zip $paths.Portable; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "DWG Classifier" -Version $ver } }
  3 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $null = Make-Zip $paths.Portable; $null = Make-Installer $paths.Portable; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "DWG Classifier" -Version $ver } }
...
# 릴리스 경로 저장 (무결성 검증용)
$script:ReleasePath = $paths.Portable
...
# 패키지 무결성 검증 (릴리스 모드만)
# ============================================================
if ($Option -eq 2 -and (Test-Path $script:VerifyScript)) {
