...
def _load_version_info():
    ...
    try:
        if getattr(sys, "frozen", False):
            ...
            # 개발 모드: config/dwg_classifier/settings.json
...
class DwgClassifierApp:
    def __init__(self, master: tk.Tk):
        ...
        # 적응형 UI 설정 초기화
        from ui_setting import get_adaptive_ui_settings, _apply_global_fonts
        ...
        base_original_height = 330  # 5행 표준 높이 (DC only)
        base_expanded_height = int(base_original_height * 2.2)  # 관리자 모드 버튼까지 보이도록 확장
        try:
        ...
        try:
            ...
            if trial_credits != -1:
                try:
                    ...
                    messages.append(f"⚠️ 크레딧 갱신 오류: {str(e)}")
            ...
            # 결과 표시
            final_message = "\n".join(messages) if messages else "업데이트 완료"
    ...
    # ---------- UI Build ----------
    def init_ui(self):
        # 설정 파일에서 창 크기 가져오기 (DPI 폴백 제거, 설정 값만 사용)
        window_width = self.ui.get("window_width", 580)
        window_height = self.ui.get("window_height", 320)
        ...
        from ui_setting import _apply_global_fonts
...
def main():
    ...
    try:
        if getattr(sys, 'frozen', False):
            # 릴리스: 실행 파일 인접 res 폴더
            icon_candidates = [
                Path(sys.executable).parent / "res" / "DWG.ico",
                Path(sys.executable).parent / "_internal" / "res" / "DWG.ico",
            ]
        else:
            # 개발: 앱 폴더 내 res 폴더
