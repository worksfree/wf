param(
  ...
  # Workaround: coerce $Option array -> int if necessary
...
$AppName = 'conversion_verifier'
$AppDir = 'D:\drive_files\10.worksfree\10.rpa\50.data\conversion_verifier'
$SpecFile = 'D:\drive_files\10.worksfree\10.rpa\50.data\conversion_verifier\conversion_verifier.spec'
...
function Copy-Portable($src,$targetRoot,$version){ $base=Join-Path $targetRoot "${AppName}_${version}"; $portable=Join-Path $base "${AppName}_${version}_portable"; Ensure-Dir $base; if(Test-Path $portable){ Remove-Item -Recurse -Force $portable }; Copy-Item -Recurse -Force -LiteralPath $src -Destination $portable; $batchSrc=Join-Path $PSScriptRoot "..\..\10.common\create_desktop_shortcut.bat"; if(Test-Path $batchSrc){ Copy-Item -Force -LiteralPath $batchSrc -Destination $portable; Write-Host "✓ 바로가기 생성 스크립트 포함: create_desktop_shortcut.bat" -ForegroundColor Green }; @{ Base=$base; Portable=$portable } }
...
!include "MUI2.nsh"
Name "Conversion Verifier v$ver"
OutFile "$outFile"
...
$dist = Build-Onedir
$ver = Read-Version (Join-Path $AppDir 'config\Conversion_Verifier\settings.json'); if(-not $ver){ $ver='0.0.0.0' }
$paths = $null
switch($Option){
  1 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "Conversion Verifier" -Version $ver } }
  2 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $null = Make-Zip $paths.Portable; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "Conversion Verifier" -Version $ver } }
  3 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $null = Make-Zip $paths.Portable; $null = Make-Installer $paths.Portable; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "Conversion Verifier" -Version $ver } }
