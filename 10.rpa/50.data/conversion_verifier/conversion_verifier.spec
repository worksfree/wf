...
def load_and_increment_version():
    ...
    settings_file = COMMON_CONFIG_LOCAL / "conversion_verifier" / "settings.json"
...
def prepare_user_configs():
    ...
    settings_src = COMMON_CONFIG_DIR / "conversion_verifier" / "settings.json"
    ...
    # 4. policy.json 복사
    policy_src = COMMON_CONFIG_DIR / "conversion_verifier" / "policy.json"
    if policy_src.exists():
        ...
        set_hidden_attribute(target_file)
        print(f"✓ policy.json 복사 완료 (.wf_rpa/conversion_verifier/)")
...
def collect_essential_resources():
    ...
    # wf_rpa_config.json - 공통 config 폴더에서 복사 (관리자 이메일, Google Sheets 포함)
...
exe = EXE(
    ...
    upx=False,
    console=False,
    # icon="res/CV.ico",  # Old icon (16x16 only)
    icon=["res\\04_Conversion_Verifier.ico"],  # New icon (16, 32, 48, 256)
)
