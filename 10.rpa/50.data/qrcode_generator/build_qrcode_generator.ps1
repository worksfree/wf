...
$AppName = 'qrcode_generator'
$AppDir = 'D:\drive_files\10.worksfree\10.rpa\50.data\qrcode_generator'
$SpecFile = 'D:\drive_files\10.worksfree\10.rpa\50.data\qrcode_generator\qrcode_generator.spec'
$Python = 'python'
...
$paths = $null
switch($BuildType){
  1 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "QR Code Generator" -Version $ver } }
  2 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $null = Make-Zip $paths.Portable; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "QR Code Generator" -Version $ver } }
  3 { $paths = Copy-Portable -src $dist -targetRoot $Candidates -version $ver; $null = Make-Zip $paths.Portable; $null = Make-Installer $paths.Portable; $exePath = Join-Path $paths.Portable "$AppName.exe"; if (Test-Path $exePath) { Create-DesktopShortcut -ExePath $exePath -AppDisplayName "QR Code Generator" -Version $ver } }
